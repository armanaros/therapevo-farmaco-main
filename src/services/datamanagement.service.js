...
const serializeValue = (val) => {
  ...
  if (typeof val?.toDate === 'function') return { _type: 'Timestamp', _s: val.seconds, _ns: val.nanoseconds };
  if (val instanceof Date) return { _type: 'Date', _iso: val.toISOString() };
...
const deserializeValue = (val) => {
  if (val === null || val === undefined) return val;
  if (typeof val === 'object' && val._type === 'Timestamp') return new Timestamp(val._s, val._ns);
  if (typeof val === 'object' && val._type === 'Date') return new Date(val._iso);
...
export const EXPORT_SECTIONS = [
  ...
  { group: 'Medical Representatives', items: [
...
export const allExportSectionItems = () => EXPORT_SECTIONS.flatMap((g) => g.items);
...
export const exportData = async (selectedKeys, onProgress) => {
  const allItems = allExportSectionItems();
  const items = allItems.filter((s) => selectedKeys.includes(s.key));
  const output = { version: '1.0', system: 'Therapevo Farmaco', exportedAt: new Date().toISOString(), meta: {}, data: {} };
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    onProgress?.({ current: i + 1, total: items.length, label: item.label });
    const snap = await getDocs(collection(db, item.collection));
    output.data[item.key] = snap.docs.map((d) => ({ _id: d.id, ...serializeValue(d.data()) }));
...
export const importData = async (exportedJson, selectedKeys, onProgress) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    for (const record of docs) {
      ...
      const docRef = _id ? doc(db, item.collection, _id) : doc(collection(db, item.collection));
