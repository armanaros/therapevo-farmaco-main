...
// ─── Value formatters (Firestore → Excel cell) ────────────────────────────────

/** Detect timestamp-like field names so they go at the end of columns */
const isTimestampKey = (k) =>
  ['At', 'Date', 'Time', 'On'].some((s) => k.endsWith(s)) ||
  ['timestamp', 'date', 'time'].includes(k.toLowerCase());

/**
 * Convert a Firestore field value to a plain Excel-safe value.
...
/**
 * Flatten a Firestore document into a flat Excel row.
 * Order: id → regular fields (alphabetical) → timestamp fields (alphabetical)
 */
...
/** Set auto-fit column widths based on content (samples first 200 rows) */
...
// ─── Value parsers (Excel cell → Firestore) ───────────────────────────────────

/**
 * Convert an Excel cell value back to a Firestore-compatible value.
 * Recognizes: ISO datetime strings, JSON arrays/objects, booleans.
 */
const parseCell = (val) => {
...
/**
 * Export selected Firestore collections to an Excel (.xlsx) Blob.
 * Each collection becomes its own sheet. A summary "_Info" sheet is added first.
 *
...
export const exportToExcel = async (selectedKeys, onProgress) => {
  ...
  // ── Fetch all data first so we can build the Info summary ──────────────
  const collected = [];
  ...
  for (const { item, rows } of collected) {
    const sheetName = item.key.slice(0, 31); // Excel sheet name limit: 31 chars
    if (rows.length === 0) {
...
/**
 * Parse a .xlsx backup file into the same shape as the old JSON backup.
 * Returns: { system, exportedAt, meta: { [key]: { count } }, data: { [key]: row[] } }
 *
 * @param {File} file
    ...
    reader.onload = (e) => {
      try {
        ...
        for (const name of wb.SheetNames) {
          ...
          // Skip empty or placeholder sheets
...
/**
 * Import data from a parsed Excel backup into Firestore.
 * Uses setDoc with the original document ID (overwrites on conflict).
 *
 * @param {object} parsedData - result of parseExcelFile
 * @param {string[]} selectedKeys - collection keys to restore
