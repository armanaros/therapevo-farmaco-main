...
const formatCell = (val) => {
  ...
  if (typeof val?.toDate === 'function') return val.toDate().toISOString().slice(0, 19).replace('T', ' ');
...
const flattenDoc = ({ _id, ...fields }) => {
  ...
  const row = { id: _id ?? '' };
  for (const k of [...regularKeys, ...tsKeys]) row[k] = formatCell(fields[k]);
  return row;
};
...
const autoFitCols = (ws, rows) => {
  ...
  ws['!cols'] = keys.map((k) => ({ wch: Math.min(60, Math.max(k.length + 2, ...rows.slice(0, 200).map((r) => String(r[k] ?? '').length + 1))) }));
};
...
export const exportToExcel = async (selectedKeys, onProgress) => {
  ...
  const infoAoa = [
    ...
    ['System', 'Therapevo Farmaco'], ['Format Version', '1.0'],
  ...
  for (const { item, rows } of collected) {
    ...
    if (rows.length === 0) {
      const ws = XLSX.utils.aoa_to_sheet([['id'], ['(this collection has no records)']]);
      ...
      try {
        ...
        if (wb.SheetNames.includes('_Info')) {
          ...
          for (const row of infoRows) {
            if (Array.isArray(row) && row[0] === 'Exported At (UTC)') result.exportedAt = String(row[1] || '');
          }
        }
        ...
        if (!Object.keys(result.data).length) throw new Error('No data sheets found.');
        resolve(result);
...
export const importFromExcel = async (parsedData, selectedKeys, onProgress) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    for (const row of rows) {
      ...
      const cleaned = {};
      for (const [k, v] of Object.entries(fields)) { const p = parseCell(v); if (p !== null && p !== undefined) cleaned[k] = p; }
      const docRef = docId && String(docId).trim() ? doc(db, item.collection, String(docId).trim()) : doc(collection(db, item.collection));
